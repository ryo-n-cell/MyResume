...
- 「確認」「連絡」「報告」を行う場合、指示を仰ぐ形では手間をかけさせる恐れがあるので、「～として行えばよいと思いますがいかがでしょう？」のように
  **決断、選択をさせて認識の齟齬をすり合わせ**を行った。
- 上長の業務を補助をしていき、その仕事を任されるような取り組みを行った。
- 積極的なフィードバック、改善策の提供を行った。

などを行い信用を得ていきました。  
そのため、様々な業務を任せていただき、プロジェクト推進のお手伝いができることができました。
...
- チームでの業務に対する悩みがある場合、その改善策を提示を行った。

などを行いチーム運営の円滑化を図りました。  
そのため、チームホスピタリティの向上が図られ、クライアントに対してチーム全体の価値が上がるお手伝いができました。
...
- 入社後のミスマッチも防ぎたいため、お互いの時間が合いましたらインターンシップを行いたいです（内容などはご相談ください）
...
IT 業界未経験から入社。  
SES 事業で 2 プロジェクトほど経験。その他を社内業務を経験

### モバイル関連企業での光回線借用申請業務/業務アプリケーション制作(2021/08~現在)

モバイル関連企業での光回線借用業務。1 人でのオンライン参画。

プロジェクト参画時、NTT から電波回線を借りるための申請書作成時に行う参照資料と資料 W チェック、業務の手動操作が多かった。

そのため、得意とする業務アプリケーション開発を通して処理が重いコードの改修、バッチ処理を行い、W チェックなどの無駄な業務フローを省いた。

結果、**月あたり 100 時間(10h×10 人)の業務時間の短縮**を達成した。
...
プロジェクト参画時、プロパーから業務効率化、改善のためのフィードバックとツール作成を行った。

SalesForce 上にある既存のデータを使用し toC 向けに公表するデータを作成するための業務アプリケーションを作成。また、文章のみの業務リファレンスを図式にするなどの改善も行った。

結果、**年当たり 100 時間の業務効率化とプロジェクト内での業務改善率が上がった**
...
現在 Web版はβ リリースまでは完成。アプリの性質上、スマートフォンアプリとして運用したほうが良いと考え、Flutterでも開発中。
